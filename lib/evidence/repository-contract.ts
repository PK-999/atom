    ...
    metricReleases: [
      {
        ...
        technologyIds: ["fixture-technology-a", "fixture-technology-b"],
      },
    ],
    metrics: [
      {
...
export function runEvidenceRepositoryContract(
  ...
  describe("EvidenceRepository contract", () => {
    ...
    const [global, region] = snapshot.geographies;
    const [technologyB, technologyA] = snapshot.technologies;
    ...
    it("returns releases, technologies, and metric geographies in stable order", async () => {
      ...
      await expect(subject.listMetricReleases()).resolves.toMatchObject([
        { metricId: metric.id },
      ]);
      ...
      await expect(subject.listGeographies(metric.id)).resolves.toMatchObject([
        { id: global.id },
      ]);
    });
    ...
    it("returns only published observations from the release active version", async () => {
      const subject = await repository();
